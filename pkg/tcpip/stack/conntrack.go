...
type connTrackMutable struct {
	// addr is source address of the tuple. It is set at initialization,
	// then is immutable.
	addr tcpip.Address

	// port is source port of the tuple. It is set at initialization,
	// then is immutable.
	port uint16

	// protocol is network layer protocol. It is set at initialization,
	// then is immutable.
...
type connTrackImmutable struct {
	// addr is destination address of the tuple. It is set at
	// initialization, then is immutable.
	addr tcpip.Address

	// direction is direction (original or reply) of the tuple. It is set
	// at initialization, then is immutable.
	direction ctDirection

	// port is destination port of the tuple. It is set at initialization,
	// then is immutable.
	port uint16

	// protocol is transport layer protocol. It is set at initialization,
	// then is immutable.
...
type connTrackTuple struct {
	// dst is non-manipulatable part of the tuple. It is set at
	// initialization and is immutable.
	dst connTrackImmutable

	// src is manipulatable part of the tuple. It is set at initialization
	// and is immutable.
...
type ConnTrackTupleHolder struct {
	// conn is pointer to the connection tracking entry. It is set at
	// initialization and is immutable.
	conn *connTrack

	// tuple is original or reply tuple. It is set at initialization and is
	// immutable.
...
type connTrack struct {
	...
	// at initialization and is immutable.
	originalTupleHolder ConnTrackTupleHolder

	// replyTupleHolder contains tuple in reply direction. It is set at
	// initialization and is immutable.
