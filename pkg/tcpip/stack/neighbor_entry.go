...
func (e *neighborEntry) setStateLocked(next NeighborState) {
	...
	case Incomplete:
		...
		sendMulticastProbe = func() {
			...
			if err := e.linkRes.LinkAddressRequest(e.mu.neigh.Addr, e.mu.neigh.LocalAddr, "", e.nic.linkEP); err != nil {
			...
			retryCounter++
			e.mu.cancel = e.nic.stack.clock.AfterFunc(&e.mu, config.RetransmitTimer, sendMulticastProbe)
		}

		sendMulticastProbe()

	case Reachable:
		e.mu.cancel = e.nic.stack.clock.AfterFunc(&e.mu, e.nudState.ReachableTime(), func() {
			e.dispatchChangeEventLocked(Stale)
			e.setStateLocked(Stale)
		})

	case Delay:
		e.mu.cancel = e.nic.stack.clock.AfterFunc(&e.mu, config.DelayFirstProbeTime, func() {
	...
	case Probe:
		...
		sendUnicastProbe = func() {
			...
			if err := e.linkRes.LinkAddressRequest(e.mu.neigh.Addr, e.mu.neigh.LocalAddr, e.mu.neigh.LinkAddr, e.nic.linkEP); err != nil {
				e.dispatchRemoveEventLocked()
				e.setStateLocked(Failed)
				return
			}
			...
			e.mu.cancel = e.nic.stack.clock.AfterFunc(&e.mu, config.RetransmitTimer, sendUnicastProbe)
		}
	...
	case Failed:
		...
		e.mu.cancel = e.nic.stack.clock.AfterFunc(&e.mu, config.UnreachableTime, func() {
