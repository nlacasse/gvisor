...
func DefaultTables() *IPTables {
	...
	ipt := IPTables{
		...
		connectionReaperDone: make(chan bool),
	}
...
// (and the goroutine) from leaking.
func (it *IPTables) StopReapingConntrack() {
	it.connectionReaperDone <- true
}
...
func (it *IPTables) Check(hook Hook, pkt *PacketBuffer, gso *GSO, r *Route, address tcpip.Address, nicName string) bool {
	...
	// NAT rule exists.
	shouldCreateEntry := it.connections.HandlePacket(pkt, hook, gso, r)

	// Go through each table containing the hook.
	for _, tablename := range it.GetPriorities(hook) {
		// If the packet was alreay NATted by an existing conntrack
	...
	// If this is a new connection and no rule applied to it, add a no-op
	// connection tracking entry for the connection. This is useful when
	// establishing connections so that the SYN/ACK reply to an outgoing
	// SYN is delivered to the correct endpoint rather than being
